...
* **双向** ：`Channel` 是双向的，可以同时用于读和写。这意味着一个 `Channel` 可以同时读取和写入数据。
* **非阻塞模式** ：`Channel` 支持非阻塞模式，可以与 `Selector` 一起使用来实现多路复用，以便处理多个通道的I/O操作。
* **直接与操作系统交互** ：`Channel` 通常直接与底层操作系统的I/O服务交互，这使得它们更高效，尤其是对大型数据块的处理。

**Stream** ：

* **单向** ：`Stream` 是单向的，一个输入流只能读数据，一个输出流只能写数据。
* **阻塞模式** ：`Stream` 是阻塞的，I/O操作会阻塞直到完成。这在处理大量并发连接时可能效率不高。
* **高层次抽象** ：`Stream` 是面向字节的高层次抽象，封装了底层的I/O操作，使用起来相对简单但灵活性较低。

#### 2. Buffer vs Stream

**Buffer** ：

* **固定大小的内存块** ：`Buffer` 是一个固定大小的内存块，用于存储和操作数据。数据必须先写入 `Buffer`，然后才能通过 `Channel` 读取或写入。
* **状态管理** ：`Buffer` 有自己的状态（position、limit 和 capacity），需要显式管理这些状态来控制读写操作。
* **高效数据操作** ：`Buffer` 允许在内存中高效地读写数据，特别是用于大数据量和非阻塞I/O操作。

**Stream** ：

* **连续流数据** ：`Stream` 代表一个连续的数据流，数据从源头（如文件、网络）读入或写出到目标（如文件、网络）。
* **简单易用** ：`Stream` 的接口简单，不需要管理额外的状态，只需处理数据流的读取或写入。
* **较低效的数据操作** ：对于大量数据操作，`Stream` 可能不如 `Buffer` 高效，因为它们不允许随意访问和操作数据，而是逐字节处理。
