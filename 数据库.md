...
[Oracle和MySQL有哪些区别？从基本特性、技术选型、字段类型、事务、语句等角度详细对比Oracle和MySQL_mysql和oracle的区别有什么-CSDN博客](https://blog.csdn.net/qq_40991313/article/details/132077467)
...
辅助索引叶子节点存储的是主键值，而不是数据的物理地址，所以访问数据需要二次查找，推荐使用覆盖索引（通常通过组合索引来实现，当你的索引里包含要查找的全部数据），可以减少回表查询
...
[MySQL：图解MVCC到底能不能解决幻读问题？_mvcc能否解决幻读问题-CSDN博客](https://blog.csdn.net/saintmm/article/details/120784426#:~:text=%E5%9C%A8REPEATABLE-READ%E9%9A%94%E7%A6%BB%E7%BA%A7%E5%88%AB%E4%B8%8B%EF%BC%8C%E5%BF%AB%E7%85%A7%E8%AF%BB%E5%8F%AF%E4%BB%A5%E9%81%BF%E5%85%8D%E5%B9%BB%E8%AF%BB%EF%BC%8C%E4%BD%86%E6%B7%B7%E5%90%88%E4%BD%BF%E7%94%A8%E5%BF%AB%E7%85%A7%E8%AF%BB%E5%92%8C%E5%BD%93%E5%89%8D%E8%AF%BB%E6%97%B6%E5%8F%AF%E8%83%BD%E5%87%BA%E7%8E%B0%E5%B9%BB%E8%AF%BB%E3%80%82,%E8%A7%A3%E5%86%B3%E6%96%B9%E6%A1%88%E5%8C%85%E6%8B%AC%E6%8F%90%E5%8D%87%E4%BA%8B%E5%8A%A1%E9%9A%94%E7%A6%BB%E7%BA%A7%E5%88%AB%E8%87%B3SERIALIZABLE%E6%88%96%E4%BD%BF%E7%94%A8%E9%97%B4%E9%9A%99%E9%94%81%E3%80%82)
...
- 快照读：通过 MVCC 来进行控制的，在可重复读隔离级别下，普通查询是快照读，是不会看到别的事务插入的数据的，但是**并不能完全避免幻读**
  ...
  select * from test;
  sessionB:
  insert into table test values(3,'88',67);
  commited;
  sessionA:
  update test set name = '77' where age = 67;
  select * from test;
  这时候A就可以读取到了，因为update前会进行一次读取操作；
  ```
- 当前读：通过 next-key 锁（行锁 + 间隙锁-select … for update;）来解决问题
...
* **数据页** ：Buffer Pool 中主要存储的是数据库表的数据页和索引页。每个数据页通常为 16KB 大小，存储在磁盘的数据页会被读取到 Buffer Pool 中供数据库操作使用。
* **自由列表（Free List）** ：存储空闲的页面块。当新的页面需要加载到 Buffer Pool 时，会从自由列表中分配一个页面。
* **LRU 列表（Least Recently Used List）** ：Buffer Pool 使用一个 LRU 算法来管理页面的缓存。最近使用的页面会放在 LRU 列表的前面，而不常用的页面则被移到列表的后面。当 Buffer Pool 空间不足时，后面的页面可能会被替换掉。
* **Flush 列表（Flush List）** ：存储脏页，即已经被修改但尚未刷新到磁盘的数据页。InnoDB 会周期性地将这些脏页写入磁盘，以确保数据的一致性。
* **Change Buffer** ：当索引页在 Buffer Pool 中不可用时，InnoDB 会将对这些页面的更改记录在 Change Buffer 中，当需要时再将更改应用到目标页面。
...
[【MySQL】一文彻底搞懂 Redo-log 为什么要两阶段提交？_mysql redolog两阶段提交-CSDN博客](https://blog.csdn.net/weixin_63566550/article/details/129819638)
